...
# Fill between band
ax1.fill_between(left_idx, vmin[left_idx], vmax[left_idx], alpha=0.12, color='gray', zorder=1)
ax2.fill_between(right_idx, vmin[right_idx], vmax[right_idx], alpha=0.12, color='gray', zorder=1)

# Lines
for name, vals in data.items():
    s = style[name]
    ax1.plot(left_idx, vals[left_idx], s['fmt'], color=s['color'],
             markersize=s['ms'], linewidth=s['lw'], label=name, zorder=s['zorder'])
    ax2.plot(right_idx, vals[right_idx], s['fmt'], color=s['color'],
             markersize=s['ms'], linewidth=s['lw'], label=name, zorder=s['zorder'])
...
fig.suptitle('Qwen3-30B-A3B-Instruct-2507: Cumulative Activation Squared', fontsize=14, y=0.97)

plt.tight_layout()
plt.savefig('/Users/bobchenyx/Downloads/Claude/moe-0223/cumulative_act2_plot.pdf')
plt.savefig('/Users/bobchenyx/Downloads/Claude/moe-0223/cumulative_act2_plot.png')
print("Saved to cumulative_act2_plot.pdf and cumulative_act2_plot.png")
